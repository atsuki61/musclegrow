...
![MuscleGrow OGP](./public/og-image.png)
...
| 観点 | 内容 |
| --- | --- |
| 課題設定 | 筋トレ継続の障壁を「記録の面倒さ」と「成長実感の弱さ」と捉え、入力補助と可視化を中心に設計 |
| UX | ログイン前でも記録できるゲストモードを用意し、会員登録後にデータを移行 |
| 技術設計 | Next.js App Router、Server Actions、Drizzle ORM、Supabase を組み合わせたシンプルな構成 |
| 型安全性 | TypeScript、Zod、Drizzle により、フォーム入力から DB 操作まで型を意識して実装 |
| 改善経験 | 認証方式をクライアント中心からサーバーサイド取得へ見直し、不要なセッション取得を削減 |
